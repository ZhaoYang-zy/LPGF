...
def main():
    ...
    out_path1 = os.path.join(os.path.dirname(os.path.dirname(__file__)), args.result_root, args.exp_name, 'lpgf_tf', f'visual/rebuild_true_{test_num}.png')
    
    out_path2 = os.path.join(os.path.dirname(os.path.dirname(__file__)), args.result_root, args.exp_name, 'lpgf_tf', f'visual/rebuild_pred_{test_num}.png')
    # model

    model = model_sets['lpgf_tf'](args)
    ...
    plocal = plocal[0,p, args.in_len-args.rebuild_start-2, ...].cpu().detach()
    ...
    pred[:, mask_local] = plocal.permute(2,0,1).reshape(c, -1)
