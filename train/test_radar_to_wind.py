...
def main():
    ...
    path = os.path.join(root, args.result_root, args.exp_name, args.model, 'weights', f'train_best{args.exp_describ}.pt')
    ...
    with tqdm(total=len(train_dataset)) as t:
        ...
        for key in args.test_metrics:
            metrics[key] = [0 for i in range(args.in_len - args.rebuild_start - 1)]
            n_metrics[key] = [0 for i in range(args.in_len - args.rebuild_start - 1)]
        ...
        for key in args.test_metrics:
            for i in range(args.in_len - args.rebuild_start - 1):
