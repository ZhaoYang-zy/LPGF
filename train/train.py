...
def main():
    ...
    # opt
    if args.train_together and 'lpgf' in args.model:
        optimizer = torch.optim.Adam([{'params': [param for name, param in model.named_parameters() if 'global' in name], 'lr': args.lr},
                                      {'params': [param for name, param in model.named_parameters() if not 'global' in name], 'lr': args.lr*args.rate}])
        scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer,max_lr=[args.lr, args.lr*args.rate],total_steps=args.epoch, pct_start=0.03, anneal_strategy='cos')
    else:
        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
        scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer,max_lr=args.lr,total_steps=args.epoch, pct_start=0.03, anneal_strategy='cos')
    ...
    metric = Metric(args.vali_metrics)
                ...
                for key in args.vali_metrics:
                    for i in range(args.pred_len):
                        if n_metrics[key][i] > 0:
                            ...
                            add_to_board(board, f'{key}_step{i + 1}', metrics[key][i], epoch)
