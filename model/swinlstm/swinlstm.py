...
class PatchInflated(nn.Module):
    ...
    def __init__(self, in_chans, embed_dim, input_resolution, patch_size, stride=2, padding=1, output_padding=1):
        ...
        for i in range(int(math.log2(patch_size)-1)):
            self.Conv.add_module(f'conv_{i}', nn.ConvTranspose2d(in_channels=embed_dim//2**i, out_channels=embed_dim//2**(i+1), kernel_size=(3, 3),
                                       stride=stride, padding=padding, output_padding=output_padding))
            
        self.Conv.add_module(f'conv_{int(math.log2(patch_size)-1)}', nn.ConvTranspose2d(in_channels=embed_dim//(patch_size//2), out_channels=in_chans, kernel_size=(3, 3),
                                       stride=stride, padding=padding, output_padding=output_padding))
...
class SwinLSTM(nn.Module):
    def __init__(self, configs):
        ...
        self.Downsample = DownSample(img_size=configs.input_size, patch_size=configs.patch_size, in_chans=configs.in_channel,
                                     embed_dim=configs.embed_dim, depths_downsample=configs.depths_downsample,
                                     num_heads=configs.num_heads, window_size=configs.window_size)

        self.Upsample = UpSample(img_size=configs.input_size, patch_size=configs.patch_size, in_chans=configs.in_channel,
                                 embed_dim=configs.embed_dim, depths_upsample=configs.depths_upsample,
                                 num_heads=configs.num_heads, window_size=configs.window_size)
...
class SwinLSTMModel(nn.Module):
    def __init__(self, configs):
        ...
        self.model = SwinLSTM(configs)
        self.out_len = configs.pred_len
        self.num_layers = len(configs.depths_downsample)
        self.MSE_criterion = nn.MSELoss()

    def forward(self, inputs, return_loss = False):
        inputs, targets = inputs[0], inputs[1]
        num_layers = self.num_layers
        targets_len = self.out_len
        ...
        targets = torch.cat((inputs[:, 1:], targets), dim=1)

        if return_loss:
            loss = self.MSE_criterion(outputs, targets)
            return outputs[:, inputs_len - 1:, ...], loss
        else:
            return outputs[:, inputs_len - 1:, ...]
