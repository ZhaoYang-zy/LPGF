...
from model.simvp.simvp_modules import (ConvSC, ConvNeXtSubBlock, ConvMixerSubBlock, GASubBlock, gInception_ST,
...
class TAU(nn.Module):
    ...
    def __init__(self, configs, drop=0.0, drop_path=0.1):
        ...
        spatio_kernel_enc = configs.spatio_kernel_enc
        spatio_kernel_dec = configs.spatio_kernel_dec

        T = configs.in_len
        C = configs.in_channel
        OC = configs.out_channel
        ...
        self.dec = Decoder(hid_S, OC, N_S, spatio_kernel_dec, act_inplace=act_inplace)

        model_type = 'tau'
        if model_type == 'incepu':
            self.hid = MidIncepNet(T * hid_S, hid_T, N_T, groups=configs.groups)
        else:
...
class MidIncepNet(nn.Module):
    ...
    def __init__(self, channel_in, channel_hid, N2, incep_ker=[3, 5, 7, 11], groups=4, **kwargs):
